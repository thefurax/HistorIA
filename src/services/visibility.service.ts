...
export const isForbiddenForAdvisor = (visibility: string): boolean => {
  return visibility === 'foreign_private' || visibility === 'gm_private' || visibility === 'never_reveal_raw';
};

export const canActorSeeEvent = (event: Partial<Event>, actorId: string): boolean => {
  const v = event.visibility;
  if (!v || isForbiddenForAdvisor(v)) return false;
